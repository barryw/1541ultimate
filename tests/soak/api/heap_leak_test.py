...
Needs a diagnostics build (-DHEAP_DIAG=1), which is the only build carrying
GET /v1/machine:heap. Against a release image the endpoint answers 404 and
every check here skips, so the suite is safe to run anywhere.
...
from report import (  # noqa: E402
    Failure, check, check_skip, check_start, detail, format_exception, section,
    suite_fail, suite_ok)
...
class Device:
    ...
    def heap_free(self) -> int:
        status, body = self._request("GET", HEAP_PATH)
        if status != 200:
            raise Failure(f"GET {HEAP_PATH} returned HTTP {status}")
        return int(json.loads(body.decode())["free"])

    def heap_available(self) -> bool:
        status, _ = self._request("GET", HEAP_PATH)
        return status == 200

    def create_d64(self, directory: str, name: str) -> None:
        path = CREATE_PATH.format(dir=urllib.parse.quote(directory),
                                  name=urllib.parse.quote(name))
        status, body = self._request("PUT", f"{path}?diskname=LEAK")
        if status != 200:
            raise Failure(f"create_d64 {name} returned HTTP {status}: {body[:160]!r}")
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Assert that repeated REST operations do not consume heap. "
                    "Requires a -DHEAP_DIAG=1 build; skips against a release image.")
    ...
    if not dev.heap_available():
        check_skip("release build: no HEAP_DIAG endpoint, nothing to measure")
        section("summary")
        detail("skipped: needs a firmware built with -DHEAP_DIAG=1")
        suite_ok("heap_leak_test")
        return 0
    from report import check_ok
    check_ok()
